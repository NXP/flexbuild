...
if [ $1 = fixcfg ]; then
    ...
    sed -i 's/# CONFIG_DEVMEM is not set/CONFIG_DEVMEM=y/' $PKGDIR/rfs/yocto-poky/meta/recipes-core/busybox/busybox/defconfig
    sed -i 's/# CONFIG_TC is not set/CONFIG_TC=y/' $PKGDIR/rfs/yocto-poky/meta/recipes-core/busybox/busybox/defconfig
    
    # add yocto custom packages with recipes
    sudo cp -rf $FBDIR/src/misc/yocto/recipes-support/* $PKGDIR/rfs/yocto-poky/meta/recipes-support/

    # reconfigure machine serial console
    [ $DESTARCH = arm64 ] && cfgfile=qemuarm64.conf || cfgfile=qemuarm.conf
    sed -i -e 's/ttyAMA0/ttyS0/' -e 's/hvc0//' $PKGDIR/rfs/yocto-poky/meta/conf/machine/$cfgfile
fi
...
if [ $1 = fixsys ]; then
    ...
    sudo mkdir -p $RFSDIR/etc/rpm-postinsts && sudo cp -f $FBDIR/src/misc/yocto/100-sysvinit-inittab $RFSDIR/etc/rpm-postinsts
